...
class CustomOOMError(Exception):
    """自定义的 OOM 异常类"""
    def __init__(self, message="Out of Memory Error"):
...
for samples,sample_type in zip([positive_samples,negative_samples],['positive','negative']):
    OOM_path = os.path.join(result_path,f'{sample_type}_OOM_llama.json')
    if os.path.exists(OOM_path):
        ooms = list(set([key for line in open(OOM_path, 'r', encoding='utf-8') for key in json.loads(line.strip())]))
    else:
        ooms = []
    for prompt_tag in ['OR2-REV','OR2']: #,'OR3-REV',]:#'RET','RET-REV']:
        ...
        if os.path.exists(output_reuslt_path):
        ...
        sample_list = [sample for sample in sample_list if sample not in ooms]
        for user in sample_list:
            ...
            try:
                ...
                with open(OOM_path, 'a', encoding='utf-8') as f:
                    ...
                    f.write('\n')
                raise CustomOOMError("Detected OOM in heavy computation")
